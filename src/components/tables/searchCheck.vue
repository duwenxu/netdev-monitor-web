<template>
  <div class="searchPar" @keyup.enter="init">
    <Form ref="form1"  :rules="rulePro" :label-width="150">
    <template v-for='(data,index) in searchData'>{{data.name}}
      ...
      <template v-if='data.type==2'>
        <Select v-model="data.value" clearable :placeholder="data.placeholder" @on-change="handleClick(data,$event)" style="width:200px;margin-right: 15px">
          <Option  v-for='choose in data.data' :value='choose.value' :key="choose.id">{{choose.name}}</Option>
        </Select>
      </template>
      <template v-if='data.type==3'>
        <DatePicker :placeholder="data.placeholder" v-model="data.value" :style="{width: data.long == 2?'200px':'300px'}" :type="data.long == 2?'date':'datetimerange'"></DatePicker>
      </template>
                    ...
                    if(obj.type == 3){
                        if(obj.long == 1){//区间模式
                            if (obj.value.length || that.search[obj.key]) {
                                that.search[obj.timeKey[0]] = that.$xy.formatDate(obj.value[0],true)
                                that.search[obj.timeKey[1]] = that.$xy.formatDate(obj.value[1],true)
                                isSend = true
                            }
